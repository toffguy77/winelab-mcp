...
**Чего не удалось получить:** выбор конкретного магазина внутри региона.
Фронт зовёт `POST /store-finder/pos {storeId}`, сервер отвечает `405` на все
варианты (POST/GET, query/body, с CSRF и без) — похоже, режется на периметре.
Практического урона мало: цена и сток в выдаче считаются по региону, а поле
`availableInOtherPoses` даёт число магазинов с остатком.
