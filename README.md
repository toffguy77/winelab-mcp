...
| `current_store` / `set_store` | магазин сессии: посмотреть и переключить (от него зависят остатки) |
...
* «Смотри магазин на Адмирала Лазарева — что там есть под стейк»
...
Пул-реквесты приветствуются — от опечатки до нового инструмента. В `main`
напрямую не пушит никто, включая мейнтейнера: всё идёт через PR и ревью.

* [CONTRIBUTING.md](CONTRIBUTING.md) — как поднять окружение, что проверяет CI
  и какие правки не принимаются
* [SECURITY.md](SECURITY.md) — куда сообщать об уязвимостях (не в публичный issue)
* [CODE_OF_CONDUCT.md](CODE_OF_CONDUCT.md) — правила общения

Особенно ценны отчёты о том, что сайт сломал свой внутренний API: для них
заведён отдельный шаблон issue.
